...
async function upsertLeadMagnetContact(payload: LeadMagnetSubmission) {
  ...
  const response = await fetch(`${GHL_BASE_URL}/contacts/upsert`, {
    ...
    body: JSON.stringify({
      ...
      email: payload.email,
      tags: ['lead_magnet', payload.interest],
  ...
  if (!response.ok) {
    throw new Error(
      `GHL lead magnet upsert failed: ${response.status} ${JSON.stringify(data)}`
    )
  }
  ...
  if (!contactId) {
    throw new Error(
      'GHL lead magnet upsert succeeded but no contact ID was returned.'
    )
  }
...
async function updateLeadMagnetFields(
  contactId: string,
  payload: Pick<LeadMagnetSubmission, 'interest'>
) {
  const token = requiredEnv('GHL_PRIVATE_TOKEN')
  ...
  if (!response.ok) {
    throw new Error(
      `GHL lead magnet field update failed: ${response.status} ${JSON.stringify(data)}`
    )
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const contactId = await upsertLeadMagnetContact(submission)

    await updateLeadMagnetFields(contactId, {
