...
| `series`, `seriesPart` | **NOW RENDERED (2026-07-10).** `series` = human-readable series name (public); `seriesPart` = 1-indexed position (positive integer). `post-blog.mjs` writes both to frontmatter when present. `lib/blog.ts` computes `seriesTotal` at render (count of posts sharing the name) and `getSeriesContext()` derives prev/next by `seriesPart` order. The blog index shows a "`<series>` · Part N of M" badge; each post page shows a "Part N of M" label plus prev/next links. To make a set of posts a series, give them the **same `series` string** and consecutive `seriesPart` numbers — nothing else. Prefer these two fields over the older `sequence_*`/`prev_in_series`/`next_in_series` (which remain inert). |
